/// <reference path="../../../connect-poc/@types/commercetools__sync-actions/index.d.ts" />
/// <reference path="../../../connect-poc/@types-extensions/graphql-ctp/index.d.ts" />
...
import FetchCustomerCustomObjects from './fetch-channels.ctp.graphql';
import FetchCustomObjectsBasedOnContainer from './fetch-custom-object.ctp.graphql';
...
type TUseCustomObjectsFetcher = () => {
  customObjects?: TFetchCustomObjectsQuery['customObjects'];
  error?: ApolloError;
  loading: boolean;
};
...
type TUseCustomObjectsContainerFetcher = (container: string) => {
  customObjects?: TFetchCustomObjectsQuery['customObjects'];
  error?: ApolloError;
  loading: boolean;
};

export const useCustomObjectsContainerFetcher: TUseCustomObjectsContainerFetcher = (container) => {
  const { data, error, loading } = useMcQuery<
  TFetchCustomObjectsQuery
  >(FetchCustomObjectsBasedOnContainer, {
    variables: {
      container,
    },
    context: {
