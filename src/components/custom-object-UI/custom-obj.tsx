...
const CustomObjectUI = (props: TCustomObjectProps) => {
  ...
  const [options, setOptions] = useState(['additionalKmAdjustmentContainer', 'leaseContainer', 'exteriorColorContainer', 'interiorColorContainer', 'warrantyContainer', 'specificationsContainer', 'featuresContainer', 'ododmeterAdjustmentsContainer', 'leaseResidualFactorContainer']);
                ...
                {customObjects?.results?.length && customObjects?.results.map((val, key) => {
                  ...
                  return (
                    <>
                      <tr key={key}>
                        <td>{val.key}</td>
                        <td>{Object.entries(val.value).map(([key, value]) => { if (Array.isArray(value)) { return key } })}</td>
                        <td>  {Object.entries(val.value).map(([key, value]) => {
                          console.log("obj nested", value, key);
                        ...
                        {/* <td>
                              <input
                                type="text"
                                name="field2"
                                value={updatedEmail[key] || val.value}
